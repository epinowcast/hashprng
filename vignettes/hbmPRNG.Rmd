...
identity_dStep <- function(t, y, parameters, salt = NULL) with(parameters, {
  ...
  for (infector in which(y == infectious)) if (length(infectable)) {
    if (!is.null(salt)) {
      ...
      infectees <- infectable[(runif(infectable[length(infectable)]) < transmission_p)[infectable]]
    } else {
      infectees <- sample(infectable, rbinom(1, length(infectable), transmission_p))
    }
    dY[infectees] <- 1
  ...
  if (t == vax_t) {
    if (!is.null(salt)) {
      ...
      vaccinees <- infectable[(runif(N) < vaccination_p)[infectable]]
    } else {
      vaccinees <- sample(infectable, rbinom(1, length(infectable), vaccination_p))
    }
...
nonid_no_match <- system.time(
  ni_non_dt <- seq_len(samplen) |> parallel::mclapply(function(seed) data.table(
    ...
    not_i = cumsum(stepper(y0 = yinit_nonid, dFUN = nonidentity_dStep, pars = nonps, HBM = FALSE)),
    withi = cumsum(stepper(y0 = yinit_nonid, dFUN = nonidentity_dStep, pars = intps, HBM = FALSE))
...
id_no_match <- system.time(
  wi_non_dt <- seq_len(samplen) |> parallel::mclapply(function(seed) data.table(
    type = "NON", model = "ID",
    not_i = cumsum(stepper(y0 = yinit, dFUN = identity_dStep, pars = nonps, HBM = FALSE)),
    withi = cumsum(stepper(y0 = yinit, dFUN = identity_dStep, pars = intps, HBM = FALSE))
  ), mc.cores = 5) |> rbindlist(idcol = "sample")
)
...
ggplot(qtiled_dt[type != "NON"]) + aes(time) +
  facet_grid(. ~ model) +
  geom_ribbon(aes(fill = type, ymax = hi, ymin = lo), alpha = 0.25) +
  geom_line(aes(color = type, y = md)) +
  scale_color_discrete(
