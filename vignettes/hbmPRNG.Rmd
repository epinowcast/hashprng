...
hash_based_matching <- system.time(
  hbm_dt <- seq_len(100) |> lapply(function(seed) data.table(
    type = "HBM",
    not_i = cumsum(stepper(pars = nonps, seed = seed, HBM = TRUE)),
    withi = cumsum(stepper(pars = intps, seed = seed, HBM = TRUE))
  )) |> rbindlist(idcol = "sample")
)
...
print("Seed-Match-Only (SMO):")
print(seed_match_only)
print("Hash-Based Match (HBM):")
