...
m_dStep_ID <- function(t, y, parameters) {
  with(parameters, {
    ...
    # individuals
    infectable <- y == 1; infectors <- which(y == 2)
    for (infector in infectors) {
      exposed <- runif(length(y), hash = list("I", infector, t)) < transmission_p
      dY <- dY | (exposed & infectable)
    }
    incI <- sum(dY == 1)
    for (infector in infectors) {
      dY <- runif(1, hash = list("R", infector, t)) < recovery_p
    }

    # if its time for vaccination
    if (t == vax_t) {
      vaxable <- infectable & (dY != 1)
      vaccinees <- (runif(length(infectable), hash = list("V")) < vaccination_p) & vaxable
